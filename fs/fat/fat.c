	       ...
	       unsigned long maxsize, int dols)
{
	...
	printf("do_fat_read_at is called %s\n", filename);
...
{
	printf("file_fat_ls is called %s\n", dir);
		      ...
		      unsigned long maxsize)
{
	unsigned long ret;
	printf("reading %s\n", filename);
	printf("pos = %x, buffer = %x\n", pos, buffer);
	ret = do_fat_read_at(filename, pos, buffer, maxsize, LS_NO);
	return ret;
}

long file_fat_read(const char *filename, void *buffer, unsigned long maxsize)
{
	printf("file_fat_read is called\n");
...
int fat_read_file(const char *filename, void *buf, int offset, int len)
{
	...
	printf("fat_read_file is called\n");
	//printf("fat_read_file is called filename = %s, buf = %x, offset = %d, len = %d\n", filename, buf, offset, len);
...
//+++ wwj
int update_check(const char *filename, unsigned long pos, void *buffer,
	       unsigned long maxsize, int dols)
{
	char fnamecopy[2048];
	boot_sector bs;
	...
	printf("update_check is called %s\n", filename);
	if (read_bootsectandvi(&bs, &volinfo, &mydata->fatsize)) {
		debug("Error: reading boot sector\n");
		return -1;
	}

				...
				phy_interface_t interface)
{
	...
	printf("get_phy_driver---\n");

	list_for_each(entry, &phy_drivers) {
		drv = list_entry(entry, struct phy_driver, list);
		if ((drv->uid & drv->mask) == (phy_id & drv->mask)) {
			printf("drv uid:%x phy_id:%x mask:%x\n", drv->uid, phy_id, drv->mask);
			return drv;
		}
	}
					    ...
					    phy_interface_t interface)
{
	...
	printf("phy_device_create---phy_id:%d\n", phy_id);
...
static int get_phy_id(struct mii_dev *bus, int addr, int devad, u32 *phy_id)
{
	...
	printf("get_phy_id phy_reg1:%x [%x, %x, %x]\n", phy_reg, addr, devad, MII_PHYSID1);

	if (phy_reg < 0)
		return -EIO;

	*phy_id = (phy_reg & 0xffff) << 16;
	printf("get_phy_id phy_id1:%x \n", phy_id);

	/* Grab the bits from PHYIR2, and put them in the lower half */
	phy_reg = bus->read(bus, addr, devad, MII_PHYSID2);
	printf("get_phy_id phy_reg2:%x [%x, %x]\n", phy_reg, addr, MII_PHYSID2);

	if (phy_reg < 0)
		return -EIO;

	*phy_id |= (phy_reg & 0xffff);
	printf("get_phy_id phy_id2:%x \n", phy_id);
		...
		unsigned phy_mask, int devad, phy_interface_t interface)
{
	...
	printf("create_phy_by_mask  phy_mask:%x\n", phy_mask);
	while (phy_mask) {
...
{
	printf("search_for_existing_phy\n");
...
static struct phy_device *get_phy_device(struct mii_dev *bus, int addr,
					 phy_interface_t interface)
{
	printf("get_phy_device Could not get PHY for %s: addr %d\n", bus->name, addr);
...
struct phy_device *phy_find_by_mask(struct mii_dev *bus, unsigned phy_mask,
		phy_interface_t interface)
{
	printf("phy_find_by_mask Could not get PHY for %s: phy_mask:%x\n", bus->name, phy_mask);
	/* Reset the bus */
	if (bus->reset)
		bus->reset(bus);
		...
		struct eth_device *dev, phy_interface_t interface)
{
	...
	printf("phy_connect Could not get PHY for %s: addr %d\n", bus->name, addr);

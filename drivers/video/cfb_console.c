...
//+++WWJ
	debug("WWJ===============%s start dataformat = %d\n", __func__, VIDEO_DATA_FORMAT);
...
//+++WWJ
	debug("%s==========width = %d\n, x = %d, y = %d\n",
		__func__, width, x, y);
	debug("%s==========sizeof(bmp_logo_bitmap) = %d\n", sizeof(bmp_logo_bitmap));
...
//+++WWJ
	debug("defined CONFIG_SPLASH_SCREEN\n");
...
//+++WWJ
	debug("%s======logo_plot start video_fb_address = 0x%x, video_logo_ypos = %d\n", 
		__func__, video_fb_address, video_logo_ypos);
	logo_plot(video_fb_address, VIDEO_COLS,
		  video_logo_xpos, video_logo_ypos);

#ifdef CONFIG_SPLASH_SCREEN_ALIGN
//+++WWJ
	debug("defined CONFIG_SPLASH_SCREEN_ALIGN\n");
	/*
	...
	if (board_cfb_skip())
		return 0;
//+++WWJ
#ifdef CONFIG_NO_VERSION
#else
	debug("WWJ=========start print version %s\n", version_string);
...
#ifdef CONFIG_CONSOLE_EXTRA_INFO
//+++WWJ
	debug("defined CONFIG_CONSOLE_EXTRA_INFO\n");
	{
		int i, n =
			((video_logo_height -
